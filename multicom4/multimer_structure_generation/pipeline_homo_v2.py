...
class Multimer_structure_prediction_homo_pipeline_v2(config.pipeline):
    ...
    def process(self,
        ...
        for method in self.run_methods:
            ...
            if method not in self.non_af2_methods and method not in self.post_af2_methods:
                ...
                model_preset = self.get_homomer_config(predictor_config, 'model_preset')
                relax_topn_predictions = self.get_homomer_config(predictor_config, 'relax_topn_predictions')
                dropout = self.get_homomer_config(predictor_config, 'dropout')
                ...
                if method == "default_multimer":
                    ...
                    base_cmd += f"--monomer_a3ms={','.join(multimer_a3ms)} "
    ...
    def process_deepmsa(self,
        ...
        predictor_config = self.homomer_config.predictors.deepmsa2
