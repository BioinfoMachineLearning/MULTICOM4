...
class Pairwise_MMalign_qa:
    ...
    def run(self, input_dir, af_ranking_file=None):
        ...
        af_score = {}
        if af_ranking_file is not None and os.path.exists(af_ranking_file):
            af_pd = pd.read_csv(af_ranking_file)
            for i in range(len(af_pd)):
                model = af_pd.loc[i,'model']
                score = af_pd.loc[i,'confidence']
        ...
        for i in range(len(pdbs)):
            ...
            for pdb2 in pdbs:
                ...
                if f"{pdb1}_{pdb2}" in scores_dict:
                    score = scores_dict[f"{pdb1}_{pdb2}"] * af_score[pdb2]
