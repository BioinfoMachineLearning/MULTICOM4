...
class Combine_alignments_v1:
    ...
    def combine_interactions(self, methods, outdir):
        ...
        for pair_info in pairs_alignments:
            ...
            cmd = f"{self.params['hhfilter_program']} -diff 50000 -i {outfile} -o {outfile}.filt -id 90"
            os.system(cmd)
        ...
        pd.DataFrame(filter_pair_ids).to_csv(f"{outputdir}/combine_v1_interact.csv", index=False)