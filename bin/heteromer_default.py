...
from multicom4.common.protein import read_qa_txt_as_df, parse_fasta, complete_result, make_chain_id_map, PDB_CHAIN_IDS
...
def main(argv):
    ...
    contents = input_fasta_str.splitlines()
    if contents[1].find(':') > 0:
        new_input_fasta_str = []
        sequences = contents[1].rstrip('\n').split(':')
        for chain_id, sequence in zip(PDB_CHAIN_IDS, sequences):
            new_input_fasta_str += [">" + chain_id]
            new_input_fasta_str += [sequence]
        fasta_path = os.path.join(FLAGS.output_dir, os.path.basename(FLAGS.fasta_path))
        with open(fasta_path, 'w') as fw:
            fw.write('\n'.join(new_input_fasta_str))
    ...
    # if not run_multimer_structure_generation_pipeline_foldseek(params=params, fasta_path=FLAGS.fasta_path,
    #                                                            chain_id_map=chain_id_map, config_name="foldseek_iter_nop",
    #                                                            pipeline_inputs=pipeline_inputs, outdir=N6_outdir):
    #     print("Program failed in step 6 foldseek_iter_nopair")

    # if not run_multimer_structure_generation_pipeline_foldseek(params=params, fasta_path=FLAGS.fasta_path,
    #                                                            chain_id_map=chain_id_map, config_name="foldseek_iter_not",
    #                                                            pipeline_inputs=pipeline_inputs, outdir=N6_outdir):
    #     print("Program failed in step 6 foldseek_iter_notemp")
