...
# add_local_script = '/bmlfast/bml_casp15/TS_run/scripts/add_local_score.pl'
...
#         print(cmd)
#         seq = os.popen(f"perl {pdb2seq} {chain_workdir}/in/{chain_id}.pdb").read().strip()
#         print(seq)
...
def add_local_scores_v2(inpdb, outpdb):
    with open(outpdb, 'w') as fw:
        for line in open(inpdb):
            if line.startswith('ATOM'):
                bfactor = line[60:66]
                if float(bfactor) > 100.0:
                    bfactor = " 99.00"
                fw.write(line[:60] + bfactor + line[66:])
...
if __name__ == '__main__':
    ...
    add_local_scores_v2(args.outdir + '/final_pre_score.pdb', args.outdir + '/final.pdb')
