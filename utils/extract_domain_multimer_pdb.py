...
if __name__ == '__main__':
    ...
    for domain_info, start_idx in zip(args.domain_infos.split(','), args.start_idx.split(',')):
        ...
        os.system(f"perl {extract_script} {args.outdir}/{chain}.pdb {args.outdir}/{chain}.domain.pdb "
                    f"{chain_start} {chain_end}")
        os.system(f"perl {reindex_script} {args.outdir}/{chain}.domain.pdb {args.outdir}/{chain}.reindex.pdb {start_idx-1}")

    combine_pdb(args.outdir, args.outdir + '/final.pdb')
